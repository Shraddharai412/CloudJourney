	...
	if(isset($_POST['submit']))
	{
		...
		if(mysqli_num_rows($res)>0){
			...
			if(mysqli_num_rows($res)==0){
				...
				$sql4="insert into airline(AIRLINE_NAME,AIRLINE_ID) values('$airportname','$airlinesid')";
				if(mysqli_query($con,$sql4))
				{
				$flag=$flag+1;
				}
				$sql5="insert into flight(`SOURCE`, `DESTINATION`, `DEPARTURE`, `ARRIVAL`, `DURATION`, `FLIGHT_CODE`, `AIRLINE_ID`, `PRICE(BUSINESS)`, `PRICE(ECONOMY)`, `PRICE(STUDENT)`, `PRICE(DIFF)`, `DATE`)values('$source','$destination','$departure','$arrival','$duration','$flightcode','$airlinesid','$businessclass','$economyclass','$students','$diff','$date')";
				...
				echo "<script>alert('Insertion Failed')</script>";
					...
					echo "<script>alert('Flight Code should be of length less than 10')</script>";
